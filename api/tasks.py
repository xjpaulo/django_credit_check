...
def validate_credit(user_age, credit_value):
    logger_celery.debug('Validation task started.')
    try:
        time.sleep(60)
        ...
        logger_celery.debug(f'Validation task finished: {detailed_log}')
    except SoftTimeLimitExceeded as e:
        message_return = 'An error occurred when running the validation task.'
        logger_celery.critical(f'Validation task aborted: {e}')
